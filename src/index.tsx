...
class TransactionApp extends React.Component<TransactionProps, TransactionState> {
  ...
  public async updateTree(): Promise<void> {
    try {
        // EOS RAM ROW
        const tree = await this.state.rpc.get_table_rows({    
            code: "thezeostoken",
  ...
  public renderTreeButton() {
    return (
      <p className='ual-btn-wrapper'>
        <span className='ual-generic-button blue' onClick={this.updateTree}>
          {'update tree'}
        </span>
      </p>
    )
